...
class _$CacheDatabase extends CacheDatabase {
  ...
  CachedQueryDao get cachedQueries {
    return _cachedQueriesInstance ??=
        _$CachedQueryDao(database, changeListener);
  }
...
class _$CachedQueryDao extends CachedQueryDao {
  ...
  Future<CachedQueryEntity?> get(String key) async {
    return _queryAdapter.query('SELECT * FROM cached_query WHERE [key] = ?1',
        mapper: (Map<String, Object?> row) => CachedQueryEntity(
            key: row['key'] as String,
            lastModifiedOn: _optionalDateTimeConverter
                .decode(row['lastModifiedOn'] as int?),
  ...
  Future<List<CachedQueryEntity>> getAll() async {
    return _queryAdapter.queryList('SELECT * FROM cached_query',
        mapper: (Map<String, Object?> row) => CachedQueryEntity(
            key: row['key'] as String,
            lastModifiedOn: _optionalDateTimeConverter
                .decode(row['lastModifiedOn'] as int?),
...
class _$IntegrationDao extends IntegrationDao {
  ...
  Stream<List<IntegrationEntity>> streamIntegrations() {
    return _queryAdapter.queryListStream('SELECT * FROM Integration',
  ...
  Future<List<IntegrationEntity>> listIntegrations() async {
    return _queryAdapter.queryList('SELECT * FROM Integration',
        mapper: (Map<String, Object?> row) => IntegrationEntity(
            identifier: row['identifier'] as String,
            displayName: row['display_name'] as String,
            logoUrl: row['logo_url'] as String,
  ...
  Future<void> addOrReplaceIntegration(IntegrationEntity integration) async {
    await _integrationEntityInsertionAdapter.insert(
        integration, OnConflictStrategy.replace);
  }

  @override
  Future<void> addOrReplaceIntegrations(
      List<IntegrationEntity> integration) async {
...
class _$SourceDao extends SourceDao {
    ...
    this.changeListener,
        ...
        _sourceEntityInsertionAdapter = InsertionAdapter(
            ...
            (SourceEntity item) => <String, Object?>{
                  ...
                  'updated_on':
                      _optionalDateTimeConverter.encode(item.updatedOn),
                  'last_modified_on':
                      _optionalDateTimeConverter.encode(item.lastModifiedOn),
                  'etag': item.eTag
                },
  ...
  Future<List<SourceEntity>> listSources() async {
    return _queryAdapter.queryList('SELECT * FROM source',
        mapper: (Map<String, Object?> row) => SourceEntity(row['id'] as int,
            ...
            updatedOn:
                _optionalDateTimeConverter.decode(row['updated_on'] as int?),
            lastModifiedOn: _optionalDateTimeConverter
                .decode(row['last_modified_on'] as int?),
            eTag: row['etag'] as String?));
  }
  ...
  Stream<List<SourceEntity>> listSourcesStream() {
    return _queryAdapter.queryListStream('SELECT * FROM source',
        mapper: (Map<String, Object?> row) => SourceEntity(row['id'] as int,
            integrationIdentifier: row['integration_identifier'] as String,
            integrationDisplayName: row['integration_display_name'] as String?,
            deviceName: row['device_name'] as String?,
  ...
  Future<SourceEntity?> getSourceById(int id) async {
    return _queryAdapter.query('SELECT * FROM source WHERE id = ?1',
        mapper: (Map<String, Object?> row) => SourceEntity(row['id'] as int,
            integrationIdentifier: row['integration_identifier'] as String,
            integrationDisplayName: row['integration_display_name'] as String?,
            deviceName: row['device_name'] as String?,
  ...
  Stream<SourceEntity?> getSourceByIdStream(int id) {
    return _queryAdapter.queryStream('SELECT * FROM source WHERE id = ?1',
        mapper: (Map<String, Object?> row) => SourceEntity(row['id'] as int,
            integrationIdentifier: row['integration_identifier'] as String,
            integrationDisplayName: row['integration_display_name'] as String?,
            deviceName: row['device_name'] as String?,
  ...
  Future<void> deleteSource(int id) async {
    await _queryAdapter
        .queryNoReturn('DELETE FROM source WHERE id = ?1', arguments: [id]);
  }
